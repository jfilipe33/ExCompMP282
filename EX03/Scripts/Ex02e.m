...
tf = 20;               % simulation time in seconds
...
for cont = 1:tf/Ts
    ...
    cp = (eye(4)-lambda)*cp + lambda*wp(contWay,:)';
    pc.rTar = cp(1:3);
    ac.psi = cp(4);
...
%     
    ...
    if((abs(norm(pc.rTar)-norm(pc.r)) < errP)) && ((abs(ac.alpha(3)-ac.psi) < errA)) && (contWay < 3)
        contWay = contWay + 1;        
    end
